...
const ListItemSearchWL: React.FC<ItemSearchWLProps> = ({ data, clickItemWL }) => {
  const { myWls, handleAddWatchlist, handleRemoveWatchlist, stock_code } = useWls()

  return (
    <div className={`mt-3 flex h-[280px] w-[260px] flex-col overflow-auto pl-2 font-semibold text-textDefault`}>
      {data?.length ? (
        data?.map(item => (
          <div
            key={item?.id}
          ...
          >
            {/* <div className='block min-w-10 text-[14px] leading-[24px] text-textDefault-third'>{item.name}</div>
              <div className='block w-full max-w-[260px] gap-1 text-[14px] text-textDefault-fourth'>
                {item?.id?.toUpperCase()}
              </div> */}
            <button
              type='button'
            ...
            >
              {/* <div className='flex min-w-10 text-left text-[14px] leading-[24px] text-textDefault-third'>
                  Id: {item.id?.toUpperCase()}
                </div> */}
              <div className='flex w-full max-w-[300px] gap-1 text-start text-[14px] text-textDefault-third'>
...
export default ListItemSearchWL
